...
export default function App() {
    ...
    if (selectedRecipe) {
      fetchRecipeDetails(selectedRecipe.id)
        .then((data) => {
          ...
          setDetailedRecipe(formattedRecipe);
        })
  ...
  return (
    <div className="min-h-screen bg-background">
      <Header favoritesCount={favoriteRecipes.length} onFavoritesClick={toggleFavoritesView} />

      <main className="container mx-auto px-4 py-8 max-w-6xl">
        {!showFavorites && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold mb-4 bg-gradient-to-r from-[#10b981] to-[#6b46c1] bg-clip-text text-transparent">
            ...
            <div className="max-w-2xl mx-auto space-y-6">
              ...
              <div className="flex gap-4 justify-center">
                <Button onClick={searchRecipes} disabled={isLoading} size="lg" className="bg-[#6b46c1] text-white">
                  <Search className="w-5 h-5 mr-2" />
                  Find Recipes
                </Button>
                <Button onClick={surpriseMe} disabled={isLoading} variant="outline" size="lg" className="text-[#10b981] border-[#10b981]">
        ...
        <section>
          ...
          <AnimatePresence mode="wait">
            {isLoading ? (
              ...
              <motion.div key="recipes" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {displayedRecipes.map((recipe, index) => (
                  <motion.div key={recipe.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}>
                    <RecipeCard recipe={recipe} onToggleFavorite={toggleFavorite} onViewDetails={openRecipeModal} />
                  </motion.div>
                ))}
              </motion.div>
            ) : (
              <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center py-16">
                <div className="w-24 h-24 mx-auto mb-6 rounded-full bg-gradient-to-br from-[#10b981]/20 to-[#6b46c1]/20 flex items-center justify-center">
                  <ChefHat className="w-12 h-12 text-muted-foreground" />
                </div>
                <h3 className="text-xl font-semibold mb-2">{showFavorites ? "No favorites yet" : "Ready to discover recipes?"}</h3>
                <p className="text-muted-foreground max-w-md mx-auto">{showFavorites ? "Save your favorite recipes to see them here." : "Add some ingredients or try the surprise me button."}</p>
              </motion.div>
            )}
          </AnimatePresence>
        </section>
      ...
      <RecipeModal recipe={detailedRecipe ?? selectedRecipe} isOpen={isModalOpen} onClose={closeRecipeModal} onToggleFavorite={toggleFavorite} />
    </div>
  );
}
